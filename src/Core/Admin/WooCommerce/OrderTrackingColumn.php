...
class OrderTrackingColumn {
	...
	function add_wc_order_tracking_custom_column( $columns ) {
		$columns['infixs-correios-automatico-tracking-column'] = __( 'Rastreio', 'infixs-correios-automatico-tracking-column' );
		$columns['infixs-correios-automatico-actions-column'] = __( 'Correios', 'infixs-correios-automatico-tracking-column' );
		return $columns;
	}
	...
	public function generate_tracking_field( $order ) {
		$codes = $this->trackingService->list( $order->get_id() )->toArray();
		$last_code = end( $codes ) ?? false;
